...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($usernameErr) && empty($passwordErr)) {
        $query = "SELECT * FROM children WHERE c_username = '$username' AND c_password = '$password'";
        $result = $conn->query($query);

        if ($result && $result->num_rows > 0) {
            $_SESSION['logged_in'] = true;
            $userData = $result->fetch_assoc();
            $_SESSION['username'] = $userData['c_username'];

            // Add session id to track child id for better data gathering
            $_SESSION['childID'] = $userData['c_id']; 
